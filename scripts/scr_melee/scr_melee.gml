function Melee() : Weapon() constructor {
	...
	static common = function() {
		...
		instance.x = obj_player._x - lengthdir_x(instance_offset, obj_player._direction) + instance_x_offset
		instance.y = obj_player._y - lengthdir_y(instance_offset, obj_player._direction) + instance_y_offset
		instance.image_angle = _direction
		
		// behind the player when the mouse is in the upper screen area
		if ((obj_player._direction > 0) and (obj_player._direction < 180)) { instance.depth = obj_player.depth - 1 } 
		...
		if ((obj_player._direction <= 90) or (obj_player._direction >= 270)) { instance.image_yscale = lerp(instance.image_yscale, 1, 0.25) }
		// facing left when the mouse is in the left screen area
		else { instance.image_yscale = lerp(instance.image_yscale, -1, 0.25) }
		
		if (state.get_current_state() == "charging") {
			// instance_x_offset = 3
